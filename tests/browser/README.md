...
画面を実際に動かして確認する回帰テスト。PHPUnit の199件は
...
| `TSUBASA_TEAM_ID` | (未設定) | テストで書き込むチーム。**テスト用のアカウントだけが所属するチーム**を指定する。ログイン直後にこのチームに切り替え、切り替わらなければ失敗する |

**本物のデータが入った環境（確認用サーバなど）では、必ず `TSUBASA_TEAM_ID` を指定すること。**
テストは投稿・コメント・予定を作って消すので、そのチームのメンバーに通知が届き、お知らせ一覧にも残る
（2026-09-25、確認用サーバの「テストチーム」で流していたため、本物の利用者の iPhone にテストの通知が届いていた）。
確認用サーバでは `smoke:account` で作った【検証】チーム（テスト用アカウントだけが所属）を使う。
