...
| 案 | 内容 | 採らなかった理由 |
| --- | --- | --- |
| 未確認の通知の数 | 送った通知のうち、まだ確認していない数。予定の変更やコメントも含み、アプリを開いたら消える | アプリ内のタブのバッジ（未読の投稿）と数が食い違う。通知の記録を持つ表も要る |
| 両方を足す | 未読の投稿＋未確認の予定・コメントの通知 | 同じ出来事を二重に数えない処理が要り、作りが複雑になる |

**割り切った点**：予定の変更やコメントの通知が来ても、アイコンのバッジの数は増えない。
通知そのものでは気づけるが、通知を見逃した場合、アイコンには何も残らない。
運用してみて物足りなければ、上の「未確認の通知の数」に見直す。
