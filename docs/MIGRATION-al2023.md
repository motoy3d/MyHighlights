...
### 複数チーム所属時の認可の穴（移行前からの問題・未修正）

1人が複数チームに所属し、画面のプルダウンで切り替える機能を調査したところ、
チームをまたいだ認可に穴があることが分かった。**移行によって生じたものではなく
移行前からの挙動**だが、本番稼働前に把握しておく必要がある。

対象チームは `current_team_id` クッキーで決まる。このクッキーは
jsから読み書きするため暗号化対象外にしてあり、利用者がブラウザの
開発者ツールで任意の値に書き換えられる。サーバ側で「そのチームに
所属しているか」を検証していないため、以下が成立してしまう。

| 事象 | 実測結果 |
| --- | --- |
| 所属していないチームの投稿一覧が読める | 200 が返り、他チームの投稿が全件見える |
| あるチームの管理者が、所属していない別チームのメンバーを退会させられる | 200 が返り、退会日が入る。そのユーザーが他チームに居なければユーザー自体も退会扱いになる |
| 所属していないチームのクッキーで `/api/me` を叩くと500 | `UserController::getMe` が `$member->admin_flg` を null 参照する |

`MemberController::destroy` の管理者判定は
「このチームの管理者でない場合は404」というコメントが付いているが、
実際のクエリが `team_id` で絞られていないため、
どれか1つのチームの管理者であれば通ってしまう。

現状を `tests/Feature/CrossTeamAuthorizationTest.php` に記録してある。
修正する場合は、`current_team_id` が所属チームかを検証するミドルウェアを
api グループに追加し、`MemberController::destroy` の管理者判定に
`team_id` の条件を足すのが最小の対応になる。修正後は同ファイルを
