...
### 最終点検 その3（2026-09-08）— テスト網羅の妥当性を、旧サーバとの比較で測る

「テストが十分か」は「何を検出するためのテストか」で決まる。移行における
テストの目的は**旧サーバとの差を検出すること**なので、境界値・異常系を
新サーバ単体で増やしても移行の安全性は上がらない。実際に旧新で突き合わせた。

| 入力 | 旧（Laravel 5.6 / PHP 7.1 / Carbon 1） | 新（Laravel 13 / PHP 8.4 / Carbon 3） | 判定 |
| --- | --- | --- | --- |
| `/api/schedules?month=202609` | 200 | 200 | 一致 |
| `month=abc` | 例外 → 500 | 例外 → 500 | 一致（元からの不具合） |
| `month=0` / 欠落 / 配列 | 例外 → 500 | 例外 → 500 | 一致 |
| `month=999913` | 例外 → 500 | `10000-01-01` → 200 | **差あり**（Carbon 3 が繰り上げる。実害なし。SPA は常に正しい `month` を送る） |
| `/api/posts?page=-1` / `abc` / 巨大値 | — | 200（1ページ目） | Laravel のページャが吸収 |
| `keyword` に SQL 片・5000文字 | — | 200 | プレースホルダで無害 |
| 存在しないID / 非数値ID | — | 404 | 妥当 |

**結論: 異常系の挙動は旧新でほぼ一致**しており、`test_monthパラメータが無いと500になる`
のような「不具合を記録している」テストも、旧と同じ挙動を固定している点で正しい。

#### この観点で見つかった実際の欠陥（対処済み）

| 欠陥 | 影響 | 対処 |
| --- | --- | --- |
| **`sql_mode` が旧と違う**。新は `ONLY_FULL_GROUP_BY` / `NO_ZERO_IN_DATE` / `NO_ZERO_DATE` を欠いていた | 旧なら拒否される `'0000-00-00'` の挿入が通る。「ゼロ日付は存在しない」という移行前提（§2.1）の根拠が新サーバでは成立しなくなる | `configure-runtime.sh` で旧と同じ `sql_mode` に。ゼロ日付が ERROR 1292、非集約 GROUP BY が ERROR 1055 で拒否されることを確認。アプリに `groupBy` は無く、全読み取りAPI 12種と ブラウザテスト 33件が厳格化後も通ることを確認 |
| 実行環境の一致確認が「表示するだけ」だった | 一致していなくても気づけない | `configure-runtime.sh` の末尾を **assert** に変更（TZ / php.ini 4項目 / 文字セット / バッファプール / sql_mode / スワップ / certbotタイマー / キューワーカー / DBとOSの時刻の一致）。一致しなければ非0で終わる |
...
## 2.6 旧本番に直接当てた変更（2026-09-21）

切り替えを待たずに旧本番へ当てたもの。**git には入っていない**（旧サーバの作業ツリーを直接編集）。
切り替えで旧サーバは使わなくなるので、新サーバ側で同じ修正を持っていれば後始末は不要。

| 変更 | 内容 | 元に戻す方法 |
| --- | --- | --- |
| 下タブをホームインジケーターの上に出す | `public/css/safe-area-20260921.css` を新設し、`resources/views/home.blade.php` の `app.css` の直後に `<link>` を1行追加。JS は再ビルドしていない（CSS だけで、`app.js` が iPhone X〜11 で body に付ける 21px も打ち消す） | `cp ~/backup-20260921/home.blade.php /var/www/MyHighlights/resources/views/home.blade.php`。HTML は no-cache なので次にアプリを開いた時点で戻る |
| 残骸の削除 | アプリ直下の `=`（9/6 のダンプ作業で誤って作った 784 バイトのファイル）を削除 | — |

新サーバ側は `5a57bb4` で同じ修正を Vue/SCSS に入れてある（機種一覧を使う手当ては削除）。

**旧サーバで JS を再ビルドしてはいけない。** 作業ツリーには未コミットの `Settings.vue` の変更（LINE 通知の行を削除）があるが、
配信中の `public/js/app.js`（2026-04-08 ビルド）には「※現在LINE通知は使用できません」を含む別の版が入っており、
作業ツリーからは再現できない。再ビルドすると画面が読めない形で変わる。
...
| 00:35 | 本番URLでスモークテスト再実施(接続先が `52.199.130.187` であることを `curl -v` で確認)。**既存の PDF 添付を1件開いて中身が出ること**も見る(添付で最も多い種類。Google のビューアがファイルを取りに来るので、切り替え前の hosts 書き換えでは旧サーバの分を見てしまい確認にならない) | △ |
| 00:40 | `SELECT COUNT(*) FROM jobs;` が 0 であることを確認してから **キューワーカー起動** `systemctl restart tsubasa-queue`（**`start` ではなく `restart`**。ワーカーは OS の起動時に自動で立ち上がり、起動時の設定を持ち続けるので、`.env` を本番値に戻した後は入れ直さないと、切り替え前の設定＝メールを外に出さない設定のまま動く。2026-09-21 の実機確認で実際に起きた）(cron は無い。certbot は timer 登録済み) | △ |
