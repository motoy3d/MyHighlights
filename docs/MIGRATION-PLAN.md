...
```
旧サーバ --(ssh/mysqldump|gzip)--> ローカル --> S3 --> 新サーバ
```
...
- **添付ファイル 5.2GB / 15,799件の転送。** DB上は `post_attachments` が
  2,490件あるのに実体が無い状態なので、このままでは
  参照系テストの「既存の添付ファイルと画像が表示できること」が確認できない
- DNS変更（`smartj.mobi` / `www` を `52.199.118.63` へ、`tsubasademo` は削除）
