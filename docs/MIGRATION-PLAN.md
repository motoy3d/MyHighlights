...
### 問題3: DNS切り替えは夜中でも即座には終わらない

- 現在のTTLのまま切り替えると、キャッシュが切れるまで
  **旧サーバにアクセスが流れ続ける**（最大でTTLの時間）
- その間、利用者ごとに新旧どちらに繋がるか分かれる。
  旧サーバ側で投稿されたデータは新サーバに入らず、**消える**

**→ まず「Elastic IPの付け替えで済まないか」を確認する。**
現行が Elastic IP 運用なら、EIPを新インスタンスに付け替えるだけで
**切り替えは数秒、切り戻しも数秒**。DNSは一切触らない。
これが使えるなら当夜のリスクは桁違いに下がる。

```bash
# 先に「どのアカウントを見ているか」を必ず確認する
aws sts get-caller-identity

aws ec2 describe-addresses --filters "Name=public-ip,Values=52.199.130.187"
# 結果が空 = EIPではない（自動割り当てIP）→ DNS切り替えが必要
```

> **前提: 新サーバは本番と同じAWSアカウントに作ること。**
> このアプリを運用しているアカウントは、手元の作業環境から
> 既定で見えるアカウントとは別。調査コマンドは必ず
> 本番アカウントの資格情報（プロファイル）で実行すること。
> 別アカウントを見て「EIPではない」と誤判定すると、
> 使えるはずの一番安全な切り替え手段を捨てることになる。
>
> アカウントが分かれると次の制約が付く。
>
> - **Elastic IPはアカウントをまたいで付け替えられない。**
>   新サーバを別アカウントに作ると、EIP付け替えという選択肢が
>   最初から消え、DNS切り替え（＋TTL短縮と伝播待ち）が必須になる
> - **SESの検証済みドメインもアカウント単位。**
>   IAMロールによるSES送信は同一アカウント内で完結している前提。
>   別アカウントから送るにはSES側にアイデンティティポリシーを
>   足すクロスアカウント設定が要る（移行と同時にやる作業ではない）
> - Route53のホストゾーンがどちらのアカウントにあるかも確認する
>   （TTL変更とレコード切り替えの実施先になる）

DNS切り替えになる場合:

- **切り替えの2〜3日前**にTTLを60〜300秒に下げる（TTL変更自体も
  旧TTLの分だけ浸透待ちが要るため、前日では間に合わない）
- 切り替えの瞬間に**旧サーバをメンテナンスモードにして書き込みを止める**。
  伝播中に旧サーバへ来たアクセスは、動いてしまうより
  「メンテナンス中」を出す方が安全
- 切り替え完了後、TTLを元に戻す

### 問題4: 証明書のニワトリタマゴ問題

DNSが旧サーバを向いている間、新サーバは HTTP-01 チャレンジで
`tsubasa.smartj.mobi` の証明書を取得できない（検証リクエストが
旧サーバに届いてしまう）。**当夜に初めて気づくと詰む。**

取り得る手のどれかを、**事前に**選んで通しておく:

| 方法 | 条件 | 備考 |
| --- | --- | --- |
| DNS-01チャレンジ | DNSのAPIが使える（Route53等） | 本命。`certbot-dns-route53`。ワイルドカードもこれで取れる |
| 旧サーバから `/etc/letsencrypt` を丸ごとコピー | 旧サーバにsshできる | 一番簡単。有効期限が残っている証明書をそのまま使い、切り替え後に更新に切り替える |
| 切り替え後に取得 | — | 取得できるまで証明書エラーが出る。**非推奨** |

いずれの場合も、切り替え後に `certbot renew --dry-run` で
**自動更新が動く状態か**を必ず確認する（今回のトラブルの再発防止）。
...
| Elastic IPか | `aws ec2 describe-addresses` | 切り替え方式（EIP付け替え or DNS）が決まる。**別アカウントに新サーバを作るとEIP付け替えは不可** |
| Route53のホストゾーンの所在 | `aws route53 list-hosted-zones` | TTL変更とレコード切り替えの実施先 |
| DNSのホスティング先とTTL | `dig tsubasa.smartj.mobi` / レジストラ管理画面 | TTL短縮の要否と実施日 |
...
| **SES送信用のIAMロール** | 旧インスタンスのIAMロールを確認 | **新インスタンスに同じロールを付けないとメールが全滅する**（`SES_KEY`/`SES_SECRET` が空でロール依存のため）。SESの検証済みドメインと同じアカウントであることが前提 |
...
| **00:40** | **切り替え実行**（EIP付け替え、またはDNSレコード変更） | **ここから切り戻しにコストが発生** |
...
| DNS切り替え | 当夜 00:40（**EIP付け替えで済むなら そちら**） |
| 新本番開始 | 当夜 00:45〜 |
| — | **切り戻し判断期限 01:30（新規）** |
